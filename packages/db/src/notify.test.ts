...
describe('notification policy', () => {
  ...
  it('suppresses non-breaking alerts during quiet hours but permits breaking alerts', () => {
    ...
    const topic = scoreNotification(
      { userId: 'reader-1', kind: 'followed_topic', at: quietNow.toISOString() },
      prefs,
      { userId: 'reader-1', sent24h: 0 },
      policy,
      quietNow,
    )
    const breaking = scoreNotification(
      { userId: 'reader-1', kind: 'breaking', at: quietNow.toISOString() },
      prefs,
      { userId: 'reader-1', sent24h: 0 },
      policy,
      quietNow,
