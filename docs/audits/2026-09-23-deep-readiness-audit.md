...
> **Resolution status (later on 2026-09-23).** Findings 1, 2, 3, 4, 5, 6 and 8 were fixed
> independently and merged to `main` through the consolidation PR #32 — including the
> lockfile, the Payload catch-all route, the public-segment allowlist and its regression
> test, the mobile consent banner, the empty-desk empty state and the reader e2e/a11y
> repairs in §9.
>
> The **Cloudflare static-export pipeline** (the reason the deployment kept failing after
> that merge) is fixed in PR #33, which also wires `audit:env-docs`, `audit:payload-routes`
> and the new `audit-script-refs` gate into `verify:static`.
>
> Still open, and tracked in `docs/LAUNCH-READINESS-ROADMAP.md`: §7 (the `ops-crons`
> workflow patch — needs an operator with `workflow` scope, since this agent's token cannot
> write `.github/workflows/*`), §10.2 (canonical origin and the missing `www`/`admin` DNS
> records), §10.3 (the apex still serves a placeholder Worker), and the dependency majors in
> §10.4.
