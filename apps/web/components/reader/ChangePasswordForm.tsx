...
export function ChangePasswordForm({ locale }: { locale: 'ne' | 'en' }) {
  ...
  function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (newPassword.length < 8) {
      setError(
        ne
          ? 'नयाँ पासवर्ड कम्तीमा ८ अक्षरको हुनुपर्छ।'
    ...
    if (newPassword === currentPassword) {
      setError(
        ne
          ? 'नयाँ पासवर्ड हालको पासवर्डभन्दा फरक हुनुपर्छ।'
    ...
    startTransition(async () => {
      try {
        ...
        if (!response.ok) {
          ...
          setError(
            String(
              body?.message ??
                body?.error?.message ??
                (ne ? 'हालको पासवर्ड मिलेन।' : 'Current password is incorrect.'),
            ),
          )
          return
        }
        ...
        setMessage(
          ne
            ? 'पासवर्ड परिवर्तन भयो। अन्य उपकरणका सत्र बन्द गरिएका छन्।'
            : 'Password updated. Sessions on other devices were revoked.',
        )
      } catch {
