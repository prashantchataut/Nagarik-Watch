...
export function ReaderTopicOnboarding({
  ...
  const scoredTopics = useMemo(
    () => scoreOnboardingTopics(categories, catalog),
    [categories, catalog],
  )
  const coverage = useMemo(
    () =>
      onboardingCoverage(
        selected,
        scoredTopics.map((topic) => topic.slug),
      ),
  ...
  async function save() {
    ...
    const next = {
      ...current,
      categories: [...new Set([...current.categories, ...selected])],
  ...
  return (
    <section className="reader-onboarding" aria-labelledby="reader-onboarding-title">
      <div>
        ...
        <h2 id="reader-onboarding-title">
          {english ? 'Choose a few useful desks' : 'आफूलाई उपयोगी केही विभाग छान्नुहोस्'}
        </h2>
        <p>
          {english
            ? 'Your choices shape recommendations and can be changed any time.'
            : 'यी छनोटले सिफारिस मिलाउँछन् र जुनसुकै बेला बदल्न सकिन्छ।'}
        </p>
      </div>
      {consented ? (
        <>
            ...
            {scoredTopics.map(({ category, recommended }) => {
              ...
              return (
                ...
                >
                  ...
                  {recommended && !active ? (
                    <small aria-hidden="true">{english ? 'Popular' : 'चलेको'}</small>
                  ) : null}
                </button>
              )
            })}
        ...
        <button
          type="button"
          className="text-action"
          onClick={() => mergeConsent({ personalization: true })}
        >
          {english
            ? 'Allow personalization to choose topics'
            : 'विषय छान्न व्यक्तिगत अनुभव अनुमति दिनुहोस्'}
        </button>
      )}
