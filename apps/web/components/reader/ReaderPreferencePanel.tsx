...
export function ReaderPreferencePanel({ locale, categories, tags, authors }: Props) {
  ...
  const options = useMemo(() => {
    ...
    const source =
      active === 'categories'
        ? categories.map((item) => ({
            slug: item.slug,
            label: english ? item.nameEn || item.nameNe : item.nameNe,
          }))
        : active === 'tags'
          ? tags.map((item) => ({
              slug: item.slug,
    ...
    return source.filter(
      (item) =>
        !needle ||
        item.label.toLocaleLowerCase(english ? 'en' : 'ne').includes(needle) ||
  ...
  function resetPreferences() {
    const timeZone =
      Intl.DateTimeFormat().resolvedOptions().timeZone || preferences.timeZone || 'Asia/Kathmandu'
    ...
    <section className="preference-desk" aria-labelledby="preference-desk-title">
      <header className="preference-desk__header">
        <div>
          ...
          <h2 id="preference-desk-title">
            {english
              ? 'Choose what deserves your attention'
              : 'तपाईंले पछ्याउन चाहेको विषय छान्नुहोस्'}
          </h2>
          <p>
            {english
              ? 'These choices tune recommendations and notification eligibility. They never override editorial prominence or public-interest breaking news.'
              : 'यी छनोटले सिफारिस र सूचनालाई मिलाउँछन्। सम्पादकीय प्राथमिकता र सार्वजनिक महत्त्वका ब्रेकिङ समाचारलाई यसले विस्थापित गर्दैन।'}
          </p>
        </div>
      ...
      <div className="preference-desk__alerts">
        <label>
          <input
            ...
            onChange={(event) => update({ ...preferences, breaking: event.target.checked })}
          />
          <span>
            <strong>{english ? 'Breaking news' : 'ब्रेकिङ समाचार'}</strong>
            <small>
              {english
                ? 'Only verified, high-priority alerts.'
                : 'प्रमाणित र उच्च प्राथमिकताका सूचना मात्र।'}
            </small>
          </span>
        </label>
        <label>
          <input
            type="checkbox"
            checked={preferences.followedTopics}
            onChange={(event) => update({ ...preferences, followedTopics: event.target.checked })}
          />
          <span>
            <strong>{english ? 'Followed desks and topics' : 'पछ्याइएका विभाग र विषय'}</strong>
            <small>
              {english
                ? 'Updates matching your selected categories and tags.'
                : 'तपाईंले छानेका विभाग र ट्यागसँग मिल्ने अपडेट।'}
            </small>
          </span>
        </label>
        <label>
          <input
            type="checkbox"
            checked={preferences.followedAuthors}
            onChange={(event) => update({ ...preferences, followedAuthors: event.target.checked })}
          />
          <span>
            <strong>{english ? 'Followed journalists' : 'पछ्याइएका पत्रकार'}</strong>
            <small>
              {english ? 'New work from selected bylines.' : 'छानिएका पत्रकारका नयाँ सामग्री।'}
            </small>
          </span>
        </label>
        <label>
          <input
            ...
            onChange={(event) => update({ ...preferences, dailyDigest: event.target.checked })}
          />
          <span>
            <strong>{english ? 'Daily briefing' : 'दैनिक समाचार सार'}</strong>
            <small>
              {english
                ? 'A compact briefing when no urgent alert qualifies.'
                : 'तत्काल सूचना योग्य नभए दिनको संक्षिप्त समाचार सार।'}
            </small>
          </span>
        </label>
      </div>
      ...
      <div className="preference-desk__schedule">
        <div>
          <strong>{english ? 'Quiet hours' : 'शान्त समय'}</strong>
          <small>
            {english
              ? `Non-breaking alerts are held in ${preferences.timeZone || 'your local timezone'}.`
              : `${preferences.timeZone || 'स्थानीय समय क्षेत्र'} अनुसार गैर-ब्रेकिङ सूचना रोकिएर बस्छन्।`}
          </small>
        </div>
        <label>
          <span>{english ? 'From' : 'सुरु'}</span>
          <select
            value={preferences.quietStart ?? ''}
            onChange={(event) =>
              update({
                ...preferences,
                quietStart: event.target.value === '' ? null : Number(event.target.value),
              })
            }
          >
            <option value="">{english ? 'Off' : 'बन्द'}</option>
        ...
        <label>
          <span>{english ? 'Until' : 'अन्त्य'}</span>
          <select
            value={preferences.quietEnd ?? ''}
            onChange={(event) =>
              update({
                ...preferences,
                quietEnd: event.target.value === '' ? null : Number(event.target.value),
              })
            }
          >
            <option value="">{english ? 'Off' : 'बन्द'}</option>
      ...
      <div className="preference-desk__picker">
        <div
          className="preference-desk__tabs"
          role="tablist"
          aria-label={english ? 'Interest type' : 'रुचिको प्रकार'}
        >
          {(['categories', 'tags', 'authors'] as const).map((key) => (
            <button
              key={key}
              id={`reader-interest-${key}`}
              type="button"
              role="tab"
              tabIndex={active === key ? 0 : -1}
              aria-selected={active === key}
              aria-controls="reader-interest-panel"
        ...
        <div
          id="reader-interest-panel"
          role="tabpanel"
          aria-labelledby={`reader-interest-${active}`}
        >
          <input
            aria-label={
              active === 'authors'
      ...
      <div className="preference-desk__footer">
        <p className="preference-desk__note">
          {english
            ? `${preferences.categories.length + preferences.tags.length + preferences.authors.length} interests selected. Recommendation version: nw-hybrid-v2.`
            : `${preferences.categories.length + preferences.tags.length + preferences.authors.length} रुचि छानिएका छन्। सिफारिस संस्करण: nw-hybrid-v2।`}{' '}
          <Link href={`${english ? '/en' : ''}/how-recommendations-work`}>
            {english ? 'See the ranking rules.' : 'क्रमका नियम हेर्नुहोस्।'}
          </Link>
        </p>
        <button
          type="button"
          ...
          disabled={pending}
          className="text-action preference-desk__reset"
        >
          {english ? 'Reset interests and alerts' : 'रुचि र सूचना रिसेट गर्नुहोस्'}
        </button>
      </div>
    </section>
  )
