...
export function PasswordResetForm({ locale, token, invalidToken, next }: { locale: 'ne' | 'en'; token: string | null; invalidToken: boolean; next?: string | null }) {
  const ne = locale === 'ne'
  const router = useRouter()
  const [pending, startTransition] = useTransition()
  const [error, setError] = useState<string | null>(invalidToken ? (ne ? 'यो लिंक मान्य छैन वा म्याद सकिएको छ।' : 'This link is invalid or has expired.') : null)
  ...
  function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (newPassword.length < 8) {
      setError(ne ? 'पासवर्ड कम्तीमा ८ अक्षरको हुनुपर्छ।' : 'Password must be at least 8 characters.')
      return
    }
    ...
    startTransition(async () => {
      try {
        ...
        setError(ne ? 'लिंकको म्याद सकिएको हुन सक्छ। नयाँ लिंक माग्नुहोस्।' : 'The link may have expired. Request a new one.')
      }
    })
  }
  ...
  if (!token || invalidToken) {
    return (
      <div className="grid gap-4" lang={ne ? 'ne' : 'en'}>
        ...
        <Link href={ne ? '/auth/forgot-password' : '/en/auth/forgot-password'} className="inline-flex h-11 w-full items-center justify-center border border-brand bg-brand px-5 text-body font-bold text-paper hover:bg-brand-strong">
  ...
  return (
    <form onSubmit={onSubmit} className="grid gap-4" noValidate>
      ...
      <div lang={ne ? 'ne' : 'en'}>
        <PasswordField name="newPassword" label={ne ? 'नयाँ पासवर्ड' : 'New password'} autoComplete="new-password" required disabled={pending} showLabel={ne ? 'देखाउनुहोस्' : 'Show'} hideLabel={ne ? 'लुकाउनुहोस्' : 'Hide'} />
      </div>
      <div lang={ne ? 'ne' : 'en'}>
