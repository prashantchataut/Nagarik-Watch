  ...
  return (
    <form onSubmit={onSubmit} className="grid gap-4" noValidate>
      ...
      <button
        type="submit"
        disabled={pending}
        className="mt-2 inline-flex h-11 w-full items-center justify-center border border-brand bg-brand px-5 text-body font-bold text-on-brand transition-colors duration-fast ease-out-quint hover:bg-brand-strong focus:outline-none focus-visible:ring-2 focus-visible:ring-brand disabled:cursor-not-allowed disabled:opacity-60"
      >
        <span lang={ne ? 'ne' : 'en'}>
          {pending
            ? ne
              ...
              : 'Sending…'
            : ne
              ? 'सुरक्षित लिंक पठाउनुहोस्'
              : 'Send secure link'}
        </span>
      </button>
      <Link
        href={loginHref}
        className="text-center text-caption font-semibold text-brand underline-offset-2 hover:underline"
        lang={ne ? 'ne' : 'en'}
      >
        {ne ? 'लगइनमा फर्कनुहोस्' : 'Back to sign in'}
      </Link>
    </form>
  )
}
