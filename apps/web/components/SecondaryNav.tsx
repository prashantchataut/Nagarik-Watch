...
/** Compact service rail: no Latest/Trending (those live in BottomNav / desk rail). */
export function SecondaryNav({ locale }: { locale: Locale }) {
  ...
  return (
    <nav
      ...
      className="nw-masthead__secondary hidden border-b border-rule bg-surface-raised md:block"
    >
          ...
          return (
            <li key={hub.key}>
              <Link
                ...
                className={
                  active
                    ? 'inline-flex min-h-10 cursor-pointer items-center whitespace-nowrap bg-brand-tint px-3 py-2 font-bold text-brand-strong'
                    : 'inline-flex min-h-10 cursor-pointer items-center whitespace-nowrap px-3 py-2 font-semibold text-ink-soft transition-colors duration-fast ease-out-quint hover:bg-surface hover:text-ink'
                }
              >
