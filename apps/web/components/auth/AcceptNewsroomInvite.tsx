...
export function AcceptNewsroomInvite({ locale, token, signedIn }: { locale: 'ne' | 'en'; token: string | null; signedIn: boolean }) {
  ...
  const [message, setMessage] = useState<string | null>(token ? null : (ne ? 'निमन्त्रणा लिंक अपूर्ण छ।' : 'The invitation link is incomplete.'))
    ...
    startTransition(async () => {
      try {
        ...
        if (!response.ok) {
          ...
          const text = reason === 'email_mismatch'
            ? (ne ? 'तपाईंको लगइन इमेल र निमन्त्रणा इमेल मिलेन।' : 'Your signed-in email does not match the invitation.')
            : reason === 'expired'
              ? (ne ? 'यो निमन्त्रणा लिंकको म्याद सकिएको छ।' : 'This invitation has expired.')
              : (ne ? 'यो निमन्त्रणा मान्य छैन वा पहिले नै प्रयोग भइसकेको छ।' : 'This invitation is invalid or has already been used.')
          setStatus('error')
          setMessage(text)
          return
        }
