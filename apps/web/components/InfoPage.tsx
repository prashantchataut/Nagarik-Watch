...
export function InfoPageHeader({ kicker, title, lead, lang }: InfoPageHeaderProps) {
  return (
    <header className="border-b border-rule pb-3 sm:pb-4" lang={lang}>
      ...
      <p className="mt-1.5 max-w-[44rem] text-meta leading-relaxed text-ink-soft sm:text-body">
        {lead}
      </p>
    </header>
  )
...
export function InfoSection({ heading, lang, children }: InfoSectionProps) {
  return (
    <section className="max-w-prose border-b border-rule pb-5 last:border-b-0">
      ...
      <p className="mt-2.5 text-body leading-relaxed text-ink-soft">{children}</p>
    </section>
  )
}
