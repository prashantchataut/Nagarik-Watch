      ...
      <div className="border-b border-rule bg-surface-raised">
        <div className="mx-auto hidden max-w-page items-center justify-between gap-3 px-4 py-2 text-caption text-ink-soft sm:flex">
          ...
          <div className="hidden items-center gap-3 sm:flex">
            <Link href={localizeHref(locale, '/latest')} className="font-semibold text-ink-soft hover:text-brand-strong" lang={lang}>
              {locale === 'en' ? 'Latest' : 'ताजा'}
            </Link>
            <Link href={localizeHref(locale, '/contact')} className="font-semibold text-ink-soft hover:text-brand-strong" lang={lang}>
      ...
      <div className="mx-auto max-w-page px-3 py-2.5 sm:px-4 sm:py-5">
        <div className="grid grid-cols-[auto_1fr_auto] items-center gap-2 md:grid-cols-[1fr_auto_1fr]">
          ...
          <div className="hidden items-center gap-2 md:flex">
            <Link href={savedHref} className="rounded-full border border-rule px-3.5 py-2 text-meta font-semibold text-ink-soft transition-colors hover:border-brand hover:bg-brand-tint hover:text-brand-strong" lang={lang}>
              {dict.navSaved}
            </Link>
          </div>
        ...
        <div className="mx-auto max-w-page px-2 sm:px-4">
          <ul className="flex flex-nowrap items-center gap-x-1 overflow-x-auto py-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            ...
            {STATIC_HUBS.filter((hub) => ['latest', 'trending', 'utilities', 'fact-check'].includes(hub.key)).map((hub) => {
              ...
              const active = pathname === href || pathname.startsWith(`${href}/`)
              return <li key={hub.key}><NavLink href={href} active={active} lang={lang}>{locale === 'en' ? hub.titleEn : hub.titleNe}</NavLink></li>
            })}
          </ul>
        </div>
      </nav>
...
function NavLink({ href, active, lang, children }: { href: string; active?: boolean; lang?: string; children: React.ReactNode }) {
  return <Link href={href} lang={lang} aria-current={active ? 'page' : undefined} className={active ? 'inline-flex min-h-10 items-center whitespace-nowrap rounded-full bg-brand px-3.5 py-1.5 text-body font-bold text-surface' : 'inline-flex min-h-10 items-center whitespace-nowrap rounded-full px-3.5 py-1.5 text-body font-semibold text-ink-soft transition-colors duration-fast ease-out-quint hover:bg-brand-tint hover:text-brand-strong'}>{children}</Link>
}
