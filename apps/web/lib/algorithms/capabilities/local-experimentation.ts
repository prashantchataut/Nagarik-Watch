...
export const LOCAL_EXPERIMENTATION_CAPABILITIES: CapabilitySpec[] = [
  {
    ...
    run: (input) => {
      ...
      const se =
        Math.sqrt(
          pooled * (1 - pooled) * (1 / Math.max(1, exposuresA) + 1 / Math.max(1, exposuresB)),
        ) || 1
      const z = (rateB - rateA) / se
      const decisive = Math.abs(z) >= 1.96
      return okLocal(
        `sprtZ=${z.toFixed(3)} decisive=${decisive} rateA=${rateA.toFixed(3)} rateB=${rateB.toFixed(3)}`,
        {
          score: Math.abs(z),
        },
      )
  ...
  },
  {
    ...
    run: (input) => {
      ...
      return okLocal(
        `posteriorMeanA=${meanA.toFixed(4)} posteriorMeanB=${meanB.toFixed(4)} probBWins~${probBBeatsA.toFixed(3)}`,
        {
          score: probBBeatsA,
        },
      )
  ...
  },
  {
    ...
    run: (input) => {
      ...
      return okLocal(
        `currentPercent=${currentPercent} nextPercent=${nextPercent} canIncrease=${canIncrease}`,
        {
          score: nextPercent,
        },
      )
  ...
  },
  {
    ...
    run: (input) => {
      ...
      if (cohortRetention.length < 2)
        return fail('local', 'cohortRetention needs at least 2 periods')
      const ratios: number[] = []
      ...
      return okLocal(
        `avgPeriodDecay=${avgDecay.toFixed(3)} forecastNextPeriod=${forecastNext.toFixed(3)}`,
        {
          score: forecastNext,
        },
      )
  ...
  },
  {
    ...
    run: (input) => {
      ...
      return okLocal(`funnelAnomalyZ=${z.toFixed(3)} steps=${stepConversionRates.length}`, {
        score: z,
      })
    },
  },
  {
    ...
    run: (input) => {
      ...
      return okLocal(
        `hotZoneClickDensity=${score.toFixed(3)} hotZone=${hotZoneClicks}/${totalClicks}`,
        { score },
      )
    },
  },
  {
    ...
    run: (input) => {
      ...
      const score = bounced
        ? 0
        : clamp01(Math.min(1, durationSeconds / 180) * 0.6 + Math.min(1, pages / 5) * 0.4)
      return okLocal(
        `sessionQuality=${score.toFixed(3)} duration=${durationSeconds}s pages=${pages}`,
        { score },
      )
    },
  },
  {
    ...
    run: (input) => {
      ...
      return okLocal(
        `clusters=${groups.size} dominant=${dominant?.[0] ?? 'none'} (${dominant?.[1] ?? 0})`,
        {
          score: groups.size,
