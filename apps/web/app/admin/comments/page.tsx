...
import { AdminPageHeader, AdminEmptyState, AdminFilterLink, AdminCard, AdminTable } from '@/components/admin/primitives'
      ...
      <AdminCard className="overflow-hidden !p-0">
        {comments.length === 0 ? <AdminEmptyState title="यो queue खाली छ" body="छानिएको अवस्थामा कुनै टिप्पणी छैन।" /> : (
          <AdminTable minWidth="48rem">
            <thead><tr><th>Reader</th><th>Comment</th><th>Article</th><th>Risk</th><th>Status</th><th>Action</th></tr></thead>
            <tbody>{comments.map((comment) => (
              <tr key={comment.id} className="align-top">
                ...
                <td className="max-w-xl whitespace-pre-wrap text-body leading-relaxed text-ink-soft">{comment.bodyNe}</td>
                <td><Link className="font-semibold text-brand-strong hover:underline" href={`/${comment.articleCategory}/${comment.articleSlug}`} target="_blank">{comment.articleSlug}</Link></td>
                ...
                <td>
                  <span className={`admin-status admin-status--${commentStatusTone(comment.status)}`}>{comment.status}</span>
                </td>
                <td><CommentModerationActions commentId={comment.id} /></td>
              </tr>
            ))}</tbody>
          </AdminTable>
        )}
