...
async function saveIssue(formData: FormData) {
  ...
  await recordAuditEvent({
    ...
    targetId: issue.id,
    summary: `Newsletter ${issue.status}: ${issue.subject}`,
  })
...
async function addSubscriber(formData: FormData) {
  ...
  if (subscriber) {
    await recordAuditEvent({
      ...
      targetId: subscriber.email,
      summary: `Subscriber added: ${subscriber.email}`,
    })
  }
  revalidatePath('/admin/newsletter')
}
  ...
  return (
    <div>
      ...
      <section className="admin-metric-grid mb-5" aria-label="Newsletter metrics">
        ...
        <AdminMetric
          value={provider.ready ? provider.provider : 'Not configured'}
          label="Provider"
        />
      </section>
          ...
          <form action={saveIssue} className="mt-4 grid gap-3">
            ...
            <AdminCallout tone="neutral">
              <p className="text-caption text-mute">
                Audience: confirmed newsletter subscribers. Member and newsroom segments are
                disabled until their identity data is connected.
              </p>
            </AdminCallout>
            ...
            <label className="flex items-center gap-2 text-meta font-semibold text-ink-soft">
              <input name="sendNow" type="checkbox" className="size-4 accent-brand" /> Add to
              delivery queue
            </label>
              ...
              issues.map((issue) => (
                <article key={issue.id} className="rounded-lg border border-rule bg-surface p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <span className={`admin-status admin-status--${issueStatusTone(issue.status)}`}>
                      {issue.status}
                    </span>
                  </div>
                  <p className="mt-2 line-clamp-3 text-meta text-ink-soft">{issue.body}</p>
                  {issue.providerMessage ? (
                    <p className="mt-2 text-caption text-mute">{issue.providerMessage}</p>
                  ) : null}
                </article>
              ))
            ) : (
