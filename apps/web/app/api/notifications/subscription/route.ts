...
import { disablePushSubscription, savePushSubscription, type PushSubscriptionInput } from '@/lib/notifications/subscriptions'
...
export async function DELETE(request: NextRequest) {
  ...
  const fingerprint = String(body.fingerprint ?? '').trim()
  const session = await getSession().catch(() => null)
  if (!endpoint || (!session && !fingerprint)) return NextResponse.json({ error: 'Subscription identity required.' }, { status: 400 })
