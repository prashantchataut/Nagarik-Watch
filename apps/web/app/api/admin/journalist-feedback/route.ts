...
import {
  ...
  isPayloadCanonical,
  updatePayloadArticleWorkflowStage,
...
export async function POST(request: NextRequest) {
  ...
  if (!session || !EDITOR_ROLES.has(session.newsroomRole))
  ...
  const article =
    action === 'revision'
      ? isPayloadCanonical()
        ? current.articleId
          ? await getPayloadJournalistDraft(current.articleId)
          : null
        : await findArticleForAdmin(current.articleId || current.articleSlug)
      : null
  if (action === 'revision' && !article) {
    return NextResponse.json(
      { error: 'Draft content could not be loaded for the revision record.' },
      { status: 409 },
    )
  }
  ...
  const next = await setJournalistFeedback(
    ...
    action === 'revision'
      ...
      : action === 'clear'
        ? null
        : (current.revisionRequestedAt ?? null),
  )
  ...
  if (action === 'revision' && article) {
    const tagSlugs =
      'tagSlugs' in article && Array.isArray(article.tagSlugs) ? article.tagSlugs : []
    await appendJournalistDraftRevision({
      ...
      snapshot: {
        ...
        reportingLocation:
          ...
          next.reportingLocation,
        sourceNote: ('sourceNote' in article ? article.sourceNote : undefined) || next.sourceNote,
        editorPitch:
          ('editorPitch' in article ? article.editorPitch : undefined) || next.editorPitch,
        mediaReferenceUrl:
          ('mediaReferenceUrl' in article ? article.mediaReferenceUrl : undefined) ||
          next.mediaReferenceUrl,
        customHomepageText:
          ('homepageTeaserNe' in article ? article.homepageTeaserNe : undefined) ||
          next.customHomepageText,
        customSocialText:
          ('socialCopyNe' in article ? article.socialCopyNe : undefined) || next.customSocialText,
  ...
  await recordAuditEvent({
    ...
    summary:
      action === 'revision'
        ? `Revision requested for ${current.titleNe}`
        : action === 'clear'
          ? `Feedback cleared for ${current.titleNe}`
          : `Editorial note added for ${current.titleNe}`,
