...
export async function generateMetadata({
  ...
  return {
    ...
    description: ne
      ? 'प्रमाणित स्कोर र खेलकुद समाचार।'
      : 'Verified scores and sports stories.',
...
export default async function SportsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const [football, cricket, stories] = await Promise.all([
    getFootballScores(),
    getCricketScores(),
    getStories({ locale, perPage: 24 }).catch(() => ({ items: [] as StoryCardData[] })),
  ])
  const sportsStories = (stories.items as StoryCardData[])
    .filter((s) => s.category?.slug === 'sports')
    .slice(0, 8)
