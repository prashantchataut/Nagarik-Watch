  ...
  return (
    <div className="mx-auto max-w-page px-4 py-6 sm:py-8">
      <header className="border-b border-rule pb-6">
        <p className="text-meta font-semibold uppercase tracking-wide text-brand-strong" lang={en ? 'en' : 'ne'}>
        ...
        <p className="mt-2 text-meta text-mute" lang={en ? 'en' : 'ne'}>{today}</p>
      </header>
        ...
        {RASHIFAL_SIGNS.map((sign) => (
          <div key={sign.slug} className="rounded-lg border border-rule bg-surface-raised p-5">
            <div className="flex items-center gap-3 border-b border-rule pb-3">
              <span className="text-h1 text-brand" aria-hidden="true">{sign.symbol}</span>
              <div>
                <p className="font-display text-h3 font-bold text-ink" lang={en ? 'en' : 'ne'}>
                  {en ? sign.nameEn : sign.nameNe}
                </p>
                <p className="text-caption text-mute" lang={en ? 'en' : 'ne'}>{en ? sign.nameEn : sign.nameNe}</p>
              </div>
            </div>
            ...
            <div className="mt-3 flex gap-4 text-caption text-mute" lang={en ? 'en' : 'ne'}>
              <span>{en ? 'Lucky number' : 'भाग्य अंक'}: {en ? sign.luckyNumber : toDevanagari(sign.luckyNumber)}</span>
              <span>{en ? 'Lucky color' : 'भाग्य रंग'}: {en ? sign.luckyColorEn : sign.luckyColorNe}</span>
            </div>
          </div>
        ))}
      </div>
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `${prefix}/rashifal`, languages: { ne: '/rashifal', en: '/en/rashifal' } },
  }
}
