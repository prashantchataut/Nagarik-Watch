    ...
    <div className="mx-auto max-w-page px-4 py-8 sm:py-12">
      <header className="max-w-3xl border-y border-rule py-7" lang={english ? 'en' : 'ne'}>
        <p className="text-caption font-bold uppercase tracking-[0.18em] text-brand-strong">
          {english ? 'News desk' : 'समाचार विभाग'}
        </p>
        <h1 className="mt-2 font-display text-display leading-tight text-ink">{name}</h1>
        {description ? (
          <p className="mt-3 text-body-lg leading-relaxed text-ink-soft">{description}</p>
        ) : null}
      </header>
      {result.items.length ? (
        <>
        ...
        >
          {english
            ? 'No reviewed stories are published in this desk yet.'
