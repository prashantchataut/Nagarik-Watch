...
export default async function JournalistFeedbackPage({
  ...
  const feedback = (await listJournalistDraftMeta(session.userId)).filter(
    (item) => item.editorFeedback || item.revisionRequestedAt,
  )
  const roleLabel = ne
          ...
          {feedback.length ? (
            feedback.map((item) => (
              <article key={item.articleSlug}>
                ...
                {item.articleId ? (
                  <Link href={localizeHref(locale, `/journalist/articles/${item.articleId}/edit`)}>
                    {ne ? 'संशोधन खोल्नुहोस्' : 'Open revision'}
                  </Link>
                ) : null}
              </article>
            ))
            ...
            <div className="newsroom-empty">
              <strong>{ne ? 'अहिले नयाँ प्रतिक्रिया छैन' : 'No new feedback'}</strong>
              <p>
                {ne
                  ? 'सम्पादकले संशोधन वा स्वीकृति नोट पठाएपछि यहाँ देखिन्छ।'
                  : 'Revision and approval notes will appear here when an editor sends them.'}
              </p>
            </div>
          )}
        </div>
