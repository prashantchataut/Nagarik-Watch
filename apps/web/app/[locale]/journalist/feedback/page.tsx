        ...
        <div className="feedback-ledger">
          {feedback.length ? feedback.map((item) => (
            <article key={item.articleSlug}>
              <div>
                <p>{item.categorySlug} · {item.workflowStage}</p>
                <h2>{item.titleNe}</h2>
            ...
            <div className="newsroom-empty">
              ...
              <p>{ne ? 'सम्पादकले संशोधन वा स्वीकृति नोट पठाएपछि यहाँ देखिन्छ।' : 'Revision and approval notes will appear here when an editor sends them.'}</p>
            </div>
          )}
        </div>
      </main>
